...
PlayerList::PlayerList(int maxSize) {
	...
	m_Items = new playerStats[maxSize];   // allocate the storage
}
...
bool PlayerList::add(const playerStats& player) {
...
//---------------------------------------------------------------------------//
playerStats PlayerList::getNext() {
	playerStats temp;
...
void PlayerList::calculateTotalCaloriesBurned() {
	...
	while (i < m_PlayerCount) {
		m_TotalCaloriesBurned = m_TotalCaloriesBurned + m_Items[i].calculateCal();
		i++;
	}
}
//---------------------------------------------------------------------------//
